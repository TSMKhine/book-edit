...
export default function StickyDirectionButton() {
  const [ctx] = useAtom(contextAtom);
  const { writingMode } = ctx.summary.properties;

  const handleDirection = (writingDirection: string) => {
    let shapeDirection;
    if (writingDirection === WRITING_MODE.HORIZONTAL) {
      shapeDirection = [
        0, -100, -35, 1, 100, -35, 1, 100, 35, 1, -100, 35, 1, -100, -35, 7,
      ];
    } else {
      shapeDirection = [
        0, -35, -100, 1, 35, -100, 1, 35, 100, 1, -35, 100, 1, -35, -100, 7,
      ];
    }
    ctx.setNodeProperties(ctx.getSelectedNodes(), {
      writingMode: writingDirection,
      commands: shapeDirection,
    });
  };
      ...
      {({ open }) => (
        <>
          ...
          <Transition
            ...
            className="absolute bottom-full z-10 mb-0.5 h-fit w-fit min-w-min -translate-x-1/3"
          >
            <Popover.Panel>
